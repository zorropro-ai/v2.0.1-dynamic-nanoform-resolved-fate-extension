...
with sync_playwright() as p:
    ...
    page.evaluate("""()=>{
      ...
      document.querySelector('#dynamicFatePreset').innerHTML='<option>Persistent mineral oxide</option>';document.querySelector('#dynamicFateSolver').value='exact_linear';document.querySelector('#dynamicFateTrackingBasis').value='constituent_element_equivalent';
      document.querySelector('#dynamicFateRateTable').innerHTML='<table><thead><tr><th>Process</th><th>First-order rate (y⁻¹)</th></tr></thead><tbody>'+[
        ['Water: matrix-bound dissolution','0.015'],['Water: free particle → small aggregate','2.200'],['Water: small → large aggregate','0.850'],['Water: free-particle dissolution','0.080'],['Water: large-aggregate settling','3.500'],['Sediment resuspension to water','0.025'],['Sediment burial below active layer','0.040']
    ...
    page.locator('#tab-advanced .dynamic-fate-panel').screenshot(path=str(ROOT/'SCREENSHOT_v2.0.1_dynamic_fate_configuration.png'))
    page.evaluate("""()=>{
      ...
      document.querySelector('#dynamicFateKpis').innerHTML='<div class="kpi"><span>Fate mass-balance closure</span><b>100.000000%</b></div><div class="kpi"><span>Final active stock</span><b>793.82 kg</b></div><div class="kpi"><span>Dominant compartment</span><b>Active sediment</b></div><div class="kpi"><span>Dominant form</span><b>Large aggregate</b></div><div class="kpi"><span>Numerical solver</span><b>Exact linear</b></div><div class="kpi"><span>Tracking basis</span><b>Element-equivalent</b></div>';
      ...
      document.querySelector('#dynamicFateSinkTable').innerHTML='<table><thead><tr><th>External sink</th><th>Steady-state annual flux (kg/y)</th><th>Spin-up cumulative mass (kg)</th></tr></thead><tbody><tr><td>Water advective removal</td><td>12.41</td><td>113.24</td></tr><tr><td>Sediment burial</td><td>8.13</td><td>76.31</td></tr><tr><td>Soil irreversible retention</td><td>2.31</td><td>21.83</td></tr></tbody></table>';
    ...
    page.locator('#dynamicFateResultsPanel').screenshot(path=str(ROOT/'SCREENSHOT_v2.0.1_dynamic_fate_results.png'))
    ...
    page.locator('#tab-advanced .dynamic-fate-panel').screenshot(path=str(ROOT/'SCREENSHOT_v2.0.1_mobile.png'))
    browser.close()
