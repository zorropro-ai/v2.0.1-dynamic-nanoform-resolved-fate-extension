"""Headless UI, dynamic fate, and responsive integration test for K-NanoMFA v2.0.1."""
import json
import re
import shutil
...
with sync_playwright() as p:
    ...
    # Enable v1.5 state allocation and v2.0.1 dynamic fate.
    ...
    page.select_option('#dynamicFateSolver','exact_linear')
    page.select_option('#dynamicFateTrackingBasis','constituent_element_equivalent')
    ...
    assert df_static['numerical_diagnostics']['solver']=='exact_linear'
    assert df_static['tracking_basis']=='constituent_element_equivalent'
    assert 'steady_state_annual_external_sinks_kg' in df_static
    assert 'spinup_cumulative_external_sinks_kg' in df_static
    ...
    # Scenario persistence includes v2.0.1 configuration.
    scenario=page.evaluate("window.KNanoApp.scenario()")
    assert scenario['version']=='2.0.1-en'
    ...
    print(json.dumps({
      ...
      'solver':df_static['numerical_diagnostics']['solver'],
      'tracking_basis':df_static['tracking_basis'],
